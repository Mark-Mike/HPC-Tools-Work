  ...
  for (u=0;u<ldb;u++)
  {
	...
	for(i=0;i<n-1;i++)
	{
	  ...
	  for(j=i+1;j<n;j++)
	  {
		...
		for(k=0;k<lda+1;k+=10)
		{
			if ((k+9)==lda-1)
			  matrix[j][k+10] = matrix[j][k+10] - ratio*matrix[i][k+10];
			else
			{
			matrix[j][k] = matrix[j][k] - ratio*matrix[i][k];
			matrix[j][k+1] = matrix[j][k+1] - ratio*matrix[i][k+1];
			matrix[j][k+2] = matrix[j][k+2] - ratio*matrix[i][k+2];
			matrix[j][k+3] = matrix[j][k+3] - ratio*matrix[i][k+3];
			matrix[j][k+4] = matrix[j][k+4] - ratio*matrix[i][k+4];
                        matrix[j][k+5] = matrix[j][k+5] - ratio*matrix[i][k+5];
                        matrix[j][k+6] = matrix[j][k+6] - ratio*matrix[i][k+6];
			matrix[j][k+7] = matrix[j][k+7] - ratio*matrix[i][k+7];
                        matrix[j][k+8] = matrix[j][k+8] - ratio*matrix[i][k+8];
                        matrix[j][k+9] = matrix[j][k+9] - ratio*matrix[i][k+9];
			}
		}
	  }
	}
