...
//#pragma omp parallel default(none) private(i,j,k) firstprivate(ratio) shared(u,w,matrix,x,n,lda,ldb,a,b,x_matrix,nrhs)

  while (u<ldb)
  {
	//#pragma omp barrier
	//#pragma omp parallel default(none) private(i,j) shared(matrix,a,b,lda,nrhs,n,u)
	...
	while (w<n-1)
	{
	  ...
	  #pragma omp parallel default(shared) private(j,k,ratio) shared(matrix,w,lda,nrhs,n,u)
	  #pragma omp for schedule(static,1)
	  for(j=w+1;j<n;j++)
	  {
	    ratio = matrix[j][w]/matrix[w][w];
		
		#pragma omp parallel for schedule(static,1)
		for(k=0;k<lda+1;k++)
		{
			matrix[j][k] = matrix[j][k] - ratio*matrix[w][k];
		}
	  }
	  //#pragma omp single
	  w++;
